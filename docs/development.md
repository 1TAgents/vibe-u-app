...
| 模型接口 | OpenAI-compatible 流式 Chat Completions |
| 部署 | Vercel |

模型客户端保留 reasoning、正文、重试、用量、成本和取消信号。瞬时网络故障、空正文、推理空转和结构化结果解析失败都会留下事件记录。
...
存储适配器已经支持 `candidate` 与 `published` 双槽。当前验收成功后的自动晋升流程仍需补充端到端闭环测试，不能将存储能力等同于完整版本系统。
