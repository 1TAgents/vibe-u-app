 ...
 *
 * 为什么需要这道门:跑批里最高频的失败不是产品缺陷,而是 Vera 编了一个
 * 界面上并不存在的控件名(「卡片分组」「本周训练量」「露营 票数」),
 * 然后 Emma 把责任归给 Alex,Alex 只能去**猜她想要什么名字** —— 可能的名字是无限的,
 * 于是连修三轮全部落空,最后误判成「疑似测试基础设施异常」。
 *
 * 正确的做法是反转依赖:不让实现去猜测试,而是在执行前就检查测试计划里的
 * 每个 target 是否**可能**解析得到,不可能的直接回喂 Vera 重写。
