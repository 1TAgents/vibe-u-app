 ...
 * 如果再让模型汇报「我干了什么」,完成状态就会退化成不可验证的自述。
 * 这里 Cody 说「写完了 9 个文件」,事件流里就必须真的有 9 个文件。
 */
...
export function toFeed(state: RunState): FeedItem[] {
  ...
  /* --- Ida 的 QA 归因与分配:Tess 报告 → Ida 分工,组织闭环在群聊里的投影 --- */
  ...
  /* --- 静态质量审计:构建后、Tess 验收前拦截源码级问题,直接派 Cody 修 --- */
  state.audits.forEach((a, i) => {
    items.push({
      ...
      text:
        ...
        (a.reasons.length > 1 ? ` …等共 ${a.reasons.length} 条,已派 Cody 修复` : ",已派 Cody 修复"),
      tags: a.files,
      tone: "warn",
    });
  });

  /* --- 静态审计修复次数上限用完:记录后继续交 Tess 运行验证,runner 硬门兜底 --- */
  state.auditExhausted.forEach((a, i) => {
    items.push({
      ...
      at: a.at,
      kind: "system",
      text: `静态审计修复次数上限已用完,仍存在 ${a.reasons.length} 条计时器生命周期问题 —— 交给 Tess 运行验证兜底`,
      tone: "warn",
    });
  });

  /* --- 修复未生效:同一失败签名第二次复现 —— Ida 保持原责任人再修一次,第三次才升级平台 --- */
  state.fixIneffective.forEach((f, i) => {
    items.push({
      ...
      at: f.at,
      kind: "system",
      text: "同一失败在修复后原样复现(修复未生效)—— Ida 保持原责任人再修一次,累计第三次仍复现才升级平台",
      tags: f.cases,
      tone: "warn",
    });
  });
  ...
  /* --- QA 测试计划覆盖修订:runTests 前缺覆盖语义回喂 Tess 重写 --- */
  state.qaCoverageRetries.forEach((c, i) => {
    items.push({
      ...
      at: c.at,
      kind: "system",
      text: `验收测试没覆盖交付重点:${c.missing.join("、")} —— 不是产品缺陷,把缺失证据交回 Tess 重写测试计划(第 ${c.round} 轮)`,
      tags: c.missing,
      tone: "warn",
    });
  });
  ...
  state.chat.forEach((c) => {
    ...
    if (c.assessment) {
      ...
      const handoff = a.designImpact
        ? "Ida 修订 PRD,Luna 更新产品设计,再由 Archie 调整系统架构与 Cody 实现"
        : a.prdImpact
          ? "Ida 先修订 PRD,Luna 更新视觉方案,随后交给 Cody 实现"
          : a.kind === "visual"
            ? "不改 PRD 和架构,交给 Luna 调整视觉方案,再由 Cody 实现"
            : "不改 PRD 和架构,直接交给 Cody 实现";
      items.push({
        id: `u${c.turn}-route`,
        seq: c.routeSeq ?? c.seq + 0.25,
        at: c.at + 1,
        kind: "system",
        text: `Ida 判断:${reason}；${handoff}；Tess ${a.qaMode === "smoke" ? "执行既有用例冒烟回归" : "重写受影响用例并回归"}`,
      });
    }
    if (c.summary) {
      const stopped = c.outcome === "stopped" || (!c.outcome && c.summary.includes("已停止"));
      items.push({
        id: `u${c.turn}-done`,
        ...
        kind: "system",
        text: c.summary,
        // 未完成时把文件标签收起来；它们是开发审计信息，不应在老板视图里制造
        // “这些文件已经改好”的错觉。成功时也只展示本轮真实差异。
        tags: stopped ? undefined : c.changed,
...
/** Ida 对一轮 QA 失败归因后在群里说的话 —— 全部由事件推导,不自证。 */
function triageText(t: QaTriage): string {
  ...
  const msg: Record<QaTriage["cause"], string> = {
    ...
    requirements: `Tess 报回的失败是验收口径/需求问题 —— ${reason};我来修订 PRD,再交 Cody 实现、Tess 回归`,
  };
  return msg[t.cause];
}
